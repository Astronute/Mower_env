...
namespace allsubscriber{
    ...
    void AllSubscriber::poseCallback(
        ...
        SysTimePoint msg_sys_time = common::protoToSystime(msg->header().stamp());
        ...
        if(last_message_times_[topic_name] <= msg_sys_time){
            ...
            common::quatToRPY(msg->pose().pose().orientation(), roll, pitch, yaw);
            ...
            last_message_times_[topic_name] = common::protoToSystime(msg->header().stamp());
            double last_update_delta = common::toSec(this->now() - msg_sys_time);
            ...
            std::cout << "Message is too old. Last message time for " << topic_name << " is " <<
                common::toSec(last_message_times_[topic_name]) << ", current message time is " << common::toSec(msg_sys_time) << std::endl;
        }
    }
    ...
    void AllSubscriber::trajectoryCallback(
        ...
        std::lock_guard<std::mutex> lock(trajectory_mtx_);
        if(last_message_times_[topic_name] <= msg_sys_time){
    ...
    void AllSubscriber::periodicUpdate(){
        ...
        if(!measurement_queue_.empty()){
            ...
            double last_update_delta = common::toSec(current_time - last_measurement_time_);
            if(last_update_delta >= sensor_dead_){
    ...
    bool AllSubscriber::getState(std::array<double, STATE_SIZE> & state){
        ...
        std::cout << "sensor delayed: " << sensor_delayed << std::endl;
        return false;
    }
    ...
    bool AllSubscriber::getLocalTrajectory(std::vector<TrajPoint>& traj){
        std::lock_guard<std::mutex> lock(trajectory_mtx_);
        int path_delayed = local_path_delayed_.load();
        if(!trajectory_queue_.empty() && path_delayed == 0){
            traj = trajectory_queue_.back();
            return true;
        }
        std::cout << "last trajectory delayed: " << path_delayed << std::endl;
        return false;
    }
